...
def run_calibration(server=None, prod_port=None, cons_port=None):
    ...
    with (open("calibration_SU.csv") as file): # Define per crop #
        ...
        for row in reader:
            ...
            for i in range(1, 24):
                yield_t = float(row[i])
                crop_to_observations["SU"].append({ # Define per crop #
                    "id": id,
                    "year": 2000 + i - 1,
    ...
    params = []
    with open("calibratethese_SU.csv") as params_csv: # Define per crop #
    ...
    # read weights
    weights = {}
    with open("Weights_SU.csv") as weights_csv: # Define per crop #
