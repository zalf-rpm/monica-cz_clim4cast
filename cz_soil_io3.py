...
def create_layer(row, prev_depth, only_raw_data, no_units=False):
    ...
    if row["sceleton"] is not None:
        layer["Sceleton"] = add_units(float(row["sceleton"]), "vol% [0-1]")
    ...
    if row["soil_organic_matter"] is not None:
        layer["SoilOrganicMatter"] = add_units(float(row["soil_organic_matter"]), "mass% [0-1]")
    elif not only_raw_data and row["soil_organic_carbon"] is not None:
        layer["SoilOrganicMatter"] = add_units(organic_carbon_to_organic_matter(float(row["soil_organic_carbon"])), "mass% [0-1]")
