...
def run_calibration(server=None, prod_port=None, cons_port=None):
    ...
    with (open("calibration_OA.csv") as file): # Define per crop #
        ...
        for row in reader:
            ...
            for i in range(1, 24):
                yield_t = float(row[i])
                crop_to_observations["OA"].append({ # Define per crop #
                    "id": id,
                    "year": 2000 + i - 1,
    ...
    params = []
    with open("calibratethese_OA.csv") as params_csv: # Define per crop #
    ...
    # read weights
    weights = {}
    with open("Weights_OA.csv") as weights_csv: # Define per crop #
